...
class Dragon:
	...
	def attack(self, finished, GameState):
		...
		if Window.Event.wait(LONGWAIT) is True:
			...
			if Window.command == "0":
				...
				if Window.Event.wait(LONGWAIT) is True:
					...
					Window.displayText("The Dragon attacks "+Player+ \
						              " and deals "+str(dam)+" damage", "<", 1)
					if Player == "themself":
						self.health -= dam
						if self.health <= 0:
							finished.set()
		...
		Window.displayText(self.name +" has "+\
			               str(self.health)+" health left.", "<", 1)
		if self.health <= 0:
			finished.set()
		return
	...
	def finishedBattle(self,GameState):
		Window = GameState.Window()
		if self.health <= 0:
			Window.displayText("The dragon is dead. You loot its corpse. "\
				              "You find 100 gold for everyone!!"\
				              "There's a note in the dragon's pocket.", \
				              ">>", 2)
			Window.displayText("It reads: 'The Princess is "\
				               "in another castle.'", "", 2)
			...
			with GameState.Lock():
				for (n, person) in People.items():
					if person.fighter:
						person.Money += 100
						Window.displayText(n + " leaves with "+\
							              str(person.Money) + " gold!", "", 2)
		elif self.fly_away > 1:
			Window.displayText("You see the dragon soaring away."\
				            "Your weapons don't reach and your taunts "\
				            "do no damage. FLuttering down is a note.", ">>", 2)
			...
			Window.displayText("The Dragon looks at your "\
				               "corpses disappointingly.", ">>", 2)
			Window.displayText("How vexing. I thought "\
				               "they were better...", "Dragon", 2)
			Window.displayText("The Dragon shapes back "\
				               "into the Old Man.", ">>", 2)
			Window.displayText("Well, time to find a "\
				               "stronger group! Hehehehe...", "The Old Man", 2)
			Window.displayText("", "", 1)
			Window.displayText("", "", 1)
			Window.displayText("", "", 1)
			Window.displayText("", "", 1)
...
class Battle(object):
	...
	def prepare4battle(self, Monster, GameState):
		...
		Window.displayText("The party enters the dungeon; they smell "\
			               "something dank and rotting. They see a cracked "\
			               "path in the wall, and decide to investigate. "\
			               "Entering the small cavern, the sound of wings "\
			               "beat upwards.", ">>", 2)
		...
		Window.displayText("But they muster up their courage, "\
			               "and begin to fight!", ">>", 2)
		...
		for (n, p) in People.items():
			if p.fighter:
				if not p.zombie and p.health <= 0:
					...
					threads.append(threading.Thread(target=p.attack,
					 args=(self.finished, Monster, GameState,)))
		for t in threads:
			t.start()
		for t in threads:
			t.join()
